#!/bin/bash
echo "*************************************************************************************"
echo "*********END EXECUTION OF docker\utilities\database-fmm\Template.dockerfile**********"
echo "*********START EXECUTION OF docker\utilities\database-fmm\setup.sh*******************"
echo "*************************************************************************************"
...
for f in $(ls script-sql/*); do
  ...
  case "$f" in
    script-sql/batch-schema-oracle10g_TS.sql)   echo "Updating sql file $f"; \
                                                cat $f \
    ...
    script-sql/FMM_Creation_Oracle.sql)         echo "Updating sql file $f"; \
                                                cat $f \
                                                | sed -e "s%DEFINE TS_DATA = &1%DEFINE TS_DATA = $DB_TS%" \
                                                | sed -e "s%DEFINE TS_INDEX = &2%DEFINE TS_INDEX = $DB_TS%" \
