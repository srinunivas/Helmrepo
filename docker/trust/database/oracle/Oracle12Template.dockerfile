...
COPY --chown=oracle:dba shared/startup/* /docker-entrypoint-initdb.d/
COPY --chown=oracle:dba shared/setup $TRUST_DATABASE_HOME/setup
COPY --chown=oracle:dba 12c/setup.sh $TRUST_DATABASE_HOME
...
RUN echo "Pulling {{deps.database.uri}}" \
  ...
  && chown -R oracle:dba $TRUST_DATABASE_HOME \
  && DB_USER=$DB_USER \
     DB_PWD=$DB_PWD \
     DB_TS=$DB_TS \
