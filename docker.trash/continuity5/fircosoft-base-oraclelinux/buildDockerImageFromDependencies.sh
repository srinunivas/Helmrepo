...
# Get oraclelinux base reference data
# ---------------
oraclelinuxReference=$(jq -r '.imageData.dependencies[] | select(.name=="oraclelinux" and .type=="image") | .reference' $IMAGE_DATA_FILE)
oraclelinuxReferenceFileName=$(find $REFERENCES_PATH/images -name '*.json' -type f -exec grep -l '"name" : "'$oraclelinuxReference {} \;)
oraclelinuxReferenceBuildDate=$(jq -r '.image.buildDate' $oraclelinuxReferenceFileName)
oraclelinuxReferenceCommit=$(jq -r '.image.commit' $oraclelinuxReferenceFileName)
oraclelinuxReferenceName=$(jq -r '.image.name' $oraclelinuxReferenceFileName)
cp $oraclelinuxReferenceFileName $IMAGE_DATA_PATH/images
...
./buildDockerImage.sh \
  ...
  --oraclelinuxImage $oraclelinuxReferenceName
