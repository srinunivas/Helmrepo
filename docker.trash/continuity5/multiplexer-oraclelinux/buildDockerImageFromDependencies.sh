...
# ---------------
multiplexerReference=$(jq -r '.imageData.dependencies[] | select(.name=="multiplexer" and .type=="artifact") | .reference' $IMAGE_DATA_FILE)
multiplexerReferenceFileName=$(find $REFERENCES_PATH/artifacts -name '*.json' -type f -exec grep -l '"name" : "'$multiplexerReference {} \;)
multiplexerReferenceUri=$(jq -r '.artifact.uri' $multiplexerReferenceFileName)
multiplexerReferenceVersion=$(jq -r '.artifact.version' $multiplexerReferenceFileName)
cp $multiplexerReferenceFileName $IMAGE_DATA_PATH/artifacts
...
./buildDockerImage.sh \
  ...
  --multiplexerUri $multiplexerReferenceUri \
  --multiplexerVersion $multiplexerReferenceVersion
