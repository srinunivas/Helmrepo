...
# ---------------
requesterReference=$(jq -r '.imageData.dependencies[] | select(.name=="requester" and .type=="artifact") | .reference' $IMAGE_DATA_FILE)
requesterReferenceFileName=$(find $REFERENCES_PATH/artifacts -name '*.json' -type f -exec grep -l '"name" : "'$requesterReference {} \;)
requesterReferenceUri=$(jq -r '.artifact.uri' $requesterReferenceFileName)
requesterReferenceVersion=$(jq -r '.artifact.version' $requesterReferenceFileName)
cp $requesterReferenceFileName $IMAGE_DATA_PATH/artifacts
...
./buildDockerImage.sh \
  ...
  --requesterUri $requesterReferenceUri \
  --requesterVersion $requesterReferenceVersion
